...
const MembershipCheckoutForm = ({ config }) => {
  const [createPaymentIntent, { data: paymentIntentData, isLoading: isPaymentIntentLoading }] =
    useCreateMembershipPaymentIntentMutation()
  const [checkRegistrationStatus, { data: registrationData, isLoading: isRegisterLoading }] =
        ...
        <Grid2 container spacing={2}>
                    ...
                    <Fragment key={index}>
                      <Grid2 container spacing={2}>
                        {map(section.inputs, (input, inputIndex) => (
                          <Grid2 size={handleInputSizing(input)} key={inputIndex}>
                            {factory(input)}
                          </Grid2>
                        ))}
                      </Grid2>
          ...
          <Grid2 size={{ xs: 4 }}>
            <Paper sx={{ px: 2, py: 3, position: 'sticky', top: 40 }}>
              ...
              >
                {isLoading ? (
                  <CircularProgress color="secondary" sx={{ width: '26px !important', height: '26px !important' }} />
                ) : (
                  'Submit Order'
                )}
              </Button>
